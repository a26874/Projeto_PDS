document
	...
	.addEventListener("submit", async function (event) {
		...
		try {
			const apiURLPhoto =
				"https://localhost:7248/Publicacao/VerificarPublicacao";
			...
			if (!buttonCreated) {
				...
				headerTabelaReconhecidos.innerHTML =
					"Pessoas Reconhecidas: " + utentesVerificados.length;
				...
				const headerDadosAutorizacaoIdentificados =
				...
				headerTabelaDadosIdentificados.append(
					headerDadosAutorizacaoIdentificados
				);
				...
				for (let i = 0; i < utentesVerificados.length; i++) {
					...
					nomeUtilizadorInput.setAttribute(
						"id",
						"nomeUtenteIdentificado" + (i + 1)
					);
					nomeUtilizadorInput.placeholder = "Nome utilizador " + (i + 1);
					if (
						utentesVerificados[i].nome != null &&
						i < utentesVerificados.length
					)
					...
					valenciaUtilizadorInput.setAttribute(
						"id",
						"valenciaIdentificado" + (i + 1)
					);
					valenciaUtilizadorInput.placeholder = "Valencia";
					if (
						utentesVerificados[i].valencia != null &&
						i < utentesVerificados.length
					)
					...
					if (
						utentesVerificados[i].sala != null &&
						i < utentesVerificados.length
					)
					...
					if (
						utentesVerificados[i].autorizacao != null &&
						i < utentesVerificados.length
					)
				...
				sendDataToEdit.addEventListener("click", async function () {
					for (let i = 0; i < utentesVerificados.length; i++) {
						const nomeUtilizadorId = document.getElementById(
							"nomeUtenteIdentificado" + (i + 1)
						);
						const valenciaUtilizadorId = document.getElementById(
							"valenciaIdentificado" + (i + 1)
						);
						const salaUtilizadorId = document.getElementById(
							"SalaIdentificado" + (i + 1)
						);
						const autUtilizadorId = document.getElementById(
							"autIdentificado" + (i + 1)
						);
						const corUtilizadorId = document.getElementById(
							"corIdentificado" + (i + 1)
						);
						...
						if (
							auxNomeUtilizadorInput[i] == nomeUtilizadorId.value &&
				...
				headerTabelaNaoReconhecidos.innerHTML =
					"Pessoas Não Reconhecidas: " + utentesVerificar.length;
				...
				headerTabelaDadosNaoIdentificados.append(
					headerDadosNomeNaoIdentificados
				);

				const headerDadosValenciaNaoIdentificados =
					document.createElement("th");
				headerDadosValenciaNaoIdentificados.innerHTML = "Valencia";
				headerDadosValenciaNaoIdentificados.style.border = "1px solid black";
				headerTabelaDadosNaoIdentificados.append(
					headerDadosValenciaNaoIdentificados
				);
				...
				headerTabelaDadosNaoIdentificados.append(
					headerDadosSalaNaoIdentificados
				);

				const headerDadosAutorizacaoNaoIdentificados =
					document.createElement("th");
				...
				headerTabelaDadosNaoIdentificados.append(
					headerDadosAutorizacaoNaoIdentificados
				);
				...
				headerTabelaDadosNaoIdentificados.append(
					headerDadosCorNaoIdentificados
				);
				...
				for (let i = 0; i < utentesVerificar.length; i++) {
					...
					linha.append(salaTd);
					const salaUtilizadorInput = document.createElement("input");
					salaUtilizadorInput.setAttribute(
						"id",
						"SalaNaoIdentificado" + (i + 1)
					);
					salaUtilizadorInput.placeholder = "Sala";
					salaTd.append(salaUtilizadorInput);
				...
				sendCoordButtonAdd.addEventListener("click", async function () {
					for (let i = 0; i < utentesVerificar.length; i++) {
						...
						const autUtilizadorId = document.getElementById(
							"autNaoIdentificado" + (i + 1)
						);
						const corUtilizadorId = document.getElementById(
							"corNaoIdentificado" + (i + 1)
						);
						const valorCor = window.getComputedStyle(corUtilizadorId);
						const valorCorAux = valorCor.backgroundColor;
						if (
							nomeUtilizadorId.value == "" &&
							valenciaUtilizadorId.value == "" &&
							salaUtilizadorId.value == "" &&
				...
				sendPessoaDesfoque.addEventListener("click", async function () {
					...
					await enviarDadosPessoaDesfoque(
						fotoOriginal,
						nomeFotoFicheiro,
						absolutePath,
						auxListaDesfocar
...
async function enviarDadosPessoa(
	posX,
	posY,
	nome,
	valencia,
	...
	try {
		...
		formDataPessoa.append(
			"utentesPorVerificar",
			JSON.stringify(utentesVerificar)
		);
...
async function editarDadosPessoa(
	idUtilizadorBd,
	nome,
	valencia,
	sala,
	...
	try {
		...
		formDataEditPessoa.append(
			"utentesVerificados",
			JSON.stringify(utentesVerificados)
		);
...
async function enviarDadosPessoaDesfoque(
	fotoOriginal,
	nomeFotoFicheiro,
	absolutePath,
	utentesVerificar
